...
async def main_menu_kb():
    ...
    builder.row(
        ...
        InlineKeyboardButton(text="О боте ℹ", callback_data="about"),
...
async def quiz_select_grade(callback: types.CallbackQuery, state: FSMContext):
    ...
    random_questions = cursor.execute('''
        ...
        ORDER BY RANDOM()
    ''', (subject, topic, grade)).fetchall()  # Явно получаем все записи
    ...
    for q in random_questions:
        ...
        answers = cursor.execute('''
            ...
            FROM options 
            WHERE quiz_id = ?
        ''', (q[0],)).fetchall()  # Явно получаем все записи

        options = [a[0] for a in answers]
        correct_option_id = None  # Инициализация по умолчанию

        # Ищем правильный ответ
        for idx, answer in enumerate(answers):
            if answer[1] == 1:
                correct_option_id = idx
                break  # Предполагаем только один правильный ответ

        # Если правильный ответ не найден - пропускаем вопрос
    ...
    # Проверяем, есть ли валидные вопросы
    if not quiz_data:
